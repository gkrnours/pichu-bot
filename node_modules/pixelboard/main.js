...
//var io.cl   = require('memory-cache')
...
fs.readFile(path.resolve(__dirname, "board.js"),  function(err, data){ if(err) throw(err) cache.put('js', data)
})
...
this.handler = function handler(req, res){
	...
	if(req.url.match(/load$/)){
		...
		for(y=0;y<32;++y){
			for(x=0;x<32;++x){
				if(board[y][x]) cells.push([x,y,board[y][x]])
			}
		}
		res.setHeader('Content-Type', "application/json")
		res.setHeader('X-JSON', JSON.stringify({ local:local,cells:cells	}))
		res.end(null, "utf-8")
		return
	}
